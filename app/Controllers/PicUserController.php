...
class PicUserController extends BaseController
{
    protected $picModel;
    ...
    public function index()
    {
        ...
        return view('EMPLOYEE/pic/pic-IT', [
            'pics' => $pics,
    ...
    public function store()
    {
        ...
        $this->picModel->insert([
            'name' => $employee['name'],  // Ambil nama employee
            'category_id' => $this->request->getPost('category_id'),
            'status' => 'Non-Active'  // Default status
        ]);

        return redirect()->to('EMPLOYEE/pic/pic-IT')->with('success', 'PIC berhasil ditambahkan');
    }
    ...
    public function delete($id)
    {
        if ($this->picModel->find($id)) {
            ...
            return redirect()->to('settings/pic')->with('success', 'PIC berhasil dihapus');
        }

        return redirect()->to('
        EMPLOYEE/pic//pic-IT')->with('error', 'PIC tidak ditemukan');
    }
