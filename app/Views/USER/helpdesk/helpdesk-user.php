            ...
            <div class="card">
                ...
                <div class="card-body">
                    ...
                    <div class="d-flex justify-content-between align-items-center mb-3 flex-wrap">
                        ...
                        <a href="<?= base_url('USER/helpdesk/create') ?>" class="btn-create-ticket btn btn-primary">
                                ...
                                <tbody>
                                    <?php foreach ($tickets as $ticket): ?>
                                        <tr>
                                            ...
                                            <td class="text-center">
                                                <?php if (!empty($ticket['ticket_rating'])): ?>
                                                    <?php for ($i = 1; $i <= 5; $i++): ?>
                                                        ...
                                                        <?php endif; ?>
                                                    <?php endfor; ?>
                                                <?php elseif (in_array(strtolower($ticket['status']), ['done', 'closed'])): ?>
                                                    <!-- Bintang kosong, bisa diklik -->
                                                    <span class="rating-clickable" data-ticket-id="<?= $ticket['id']; ?>">
                                                        <?php for ($i = 1; $i <= 5; $i++): ?>
                                                            <i class="far fa-star text-muted"></i>
                                                        <?php endfor; ?>
                                                    </span>
                                                <?php else: ?>
                                                    <!-- Tiket belum selesai, bintang non-klik -->
                                                    <?php for ($i = 1; $i <= 5; $i++): ?>
                                                        <i class="far fa-star text-muted"></i>
                                                    <?php endfor; ?>
                                                <?php endif; ?>
                                            ...
                                            <!-- Actions -->
                                            <td>
                                                <a href="USER/helpdesk/edit/<?= $ticket['id'] ?>"
                                                    class="btn btn-sm btn-info me-2">
                                                    <i class="fas fa-edit"></i> Edit
                                                </a>
                                                <a href="/helpdesk/delete/<?= $ticket['id'] ?>"
                                    ...
                                    <form id="ratingForm">
                                        <div class="modal-content">
                                            ...
                                            <div class="modal-body">
                                                ...
                                                <label for="comment" class="mt-2">Komentar:</label>
...
<!-- Taruh JS di luar form agar bersih -->
<script>
    $('#rating').on('change', function () {
        let rating = $(this).val();

        let komentarDefault = {
            1: 'Sangat buruk, perlu banyak perbaikan.',
            2: 'Kurang memuaskan, masih banyak kekurangan.',
            3: 'Pelayanan cukup baik, namun perlu peningkatan.',
            4: 'Baik, cukup puas dengan pelayanan yang diberikan.',
            5: 'Sangat puas dengan pelayanan yang cepat dan profesional.'
        };

        $('#comment').val(komentarDefault[rating] || '');
    });
</script>

<script>
    document.addEventListener("DOMContentLoaded", function () {
        // Ketika klik bintang kosong
        document.querySelectorAll('.rating-clickable').forEach(el => {
        ...
        document.getElementById('ratingForm').addEventListener('submit', function (e) {
            ...
            fetch("<?= site_url('helpdesk/rateTicket') ?>", {
                method: 'POST',
                headers: {
                ...
                .then(response => {
                    if (response.status === 'success') {
                        Swal.fire('Berhasil!', 'Rating berhasil dikirim!', 'success').then(() => {
                            location.reload(); // Refresh biar kelihatan
                        });
                    } else {
                        Swal.fire('Gagal', response.message || 'Terjadi kesalahan.', 'error');
                    }
                })
                .catch(() => Swal.fire('Error', 'Gagal mengirim rating.', 'error'));
        });
    });
