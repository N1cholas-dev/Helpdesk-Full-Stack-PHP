...
class HelpdeskController extends BaseController
{
    ...
    public function helpdesk()
    {
        ...
        // Join Relasi
        $this->ticketModel->join('subjects', 'subjects.id = helpdesk_tickets.subject_id', 'left');
        $this->ticketModel->join('problems', 'problems.id = helpdesk_tickets.problem_id', 'left');
        $this->ticketModel->join('categories', 'categories.id = helpdesk_tickets.category_id', 'left');
        $this->ticketModel->join('subcategories', 'subcategories.id = helpdesk_tickets.subcategory_id', 'left');
        $this->ticketModel->join('departments', 'departments.id = helpdesk_tickets.department_id', 'left');
        $this->ticketModel->join('ticket_feedback', 'ticket_feedback.ticket_id = helpdesk_tickets.id', 'left');
        ...
        foreach ($tickets as &$ticket) {
            // Mengambil lampiran untuk setiap tiket
            $ticket['attachments'] = $this->ticketattachmentModel
                ->where('ticket_id', $ticket['id'])
                ->findAll();
        }
        unset($ticket); // best practice
        ...
        return view('DOCUMENTATION IT/helpdesk/helpdesk-admin', $data);
    }
    ...
    public function create()
    {
        ...
        return view('DOCUMENTATION IT/helpdesk/create', $data);
    }
    ...
    public function store()
    {
        ...
        return redirect()->to('/DOCUMENTATION IT/helpdesk/helpdesk-admin')->with('success', 'Tiket berhasil dibuat!');
    }

    public function edit($ticket_id)
{
    // Ambil data ticket berdasarkan ticket_id
    $ticket = $this->ticketModel->find($ticket_id);

    // Jika ticket tidak ditemukan, redirect dengan pesan error
    if (!$ticket) {
        return redirect()->to('DOCUMENTATION IT/helpdesk/helpdesk-admin')->with('error', 'Ticket not found!');
    }
    ...
    // Cek apakah subject dari ticket sudah ada di daftar subjects
    $currentSubject = $ticket['subject'] ?? '';
    $subjectExists = false;

    foreach ($subjects as $subject) {
        if ($subject['description'] === $currentSubject) {
            $subjectExists = true;
            break;
        }
    }

    // Jika subject belum ada, tambahkan subject yang ada pada ticket ke dalam daftar
    if (!$subjectExists && $currentSubject !== '') {
        $subjects[] = ['description' => $currentSubject];
    }

    // Menambahkan deskripsi masalah yang sesuai dengan ticket
    $currentProblem = $ticket['problem_id'] ?? '';
    $problemDescription = '';

    // Cek apakah problem ID ada, jika ada, ambil deskripsi masalah
    if ($currentProblem) {
        // Cari deskripsi masalah yang sesuai
        $problem = $problemModel->find($currentProblem);
    ...
    $data = [
        ...
        'currentProblem' => $currentProblem, // Menambahkan ID problem
        'problemDescription' => $problemDescription, // Menambahkan deskripsi problem
    ];

    // Kembalikan tampilan edit dengan data yang sudah diambil
    return view('DOCUMENTATION IT/helpdesk/edit', $data);
}
    ...
    public function update($ticket_id)
    {
        ...
        if ($this->db->transStatus() === false) {
            ...
            return redirect()->to('DOCUMENTATION IT/helpdesk/helpdesk-admin');
        }
    ...
    public function delete($id)
    {
        ...
        if ($ticket) {
            ...
            return redirect()->to('DOCUMENTATION IT/helpdesk/helpdesk-admin')->with('success', 'deleted');
        } else {
